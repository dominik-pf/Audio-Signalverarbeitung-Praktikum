...
tic
% add Bruce
carfacPath = fullfile(hubPath,'Bruce');
...
% Set audiogram
ag_fs = [250 500 1e3 2e3 4e3 8e3 11200];
...
combos
    ...
    %% Rum Bruce Model
    [~,~,neurogram_Sout_down,~,~,t_Sout_down,CF_down] = generate_neurogram_function(stim_down,Fs_stim_down,species,ag_fs,ag_dbloss);
 
    CFs_down = CF_down(:);
    
    if size(neurogram_Sout_down,1) ~= length(t_Sout_down)
        neurogram_Sout_down = neurogram_Sout_down.';
    end
    
    cf_down_mask = CFs_down >= 125 & CFs_down <= 20000;
    CFs_down_plot = CFs_down(cf_down_mask);
    neurogram_Sout_down_plot = neurogram_Sout_down(:,cf_down_mask);
    ...
    %% Rum Bruce Model
    [~,~,neurogram_Sout_up,~,~,t_Sout_up,CF_up] = generate_neurogram_function(stim_up,Fs_stim_up,species,ag_fs,ag_dbloss);
 
    CFs_up = CF_up(:);
    
    if size(neurogram_Sout_up,1) ~= length(t_Sout_up)
        neurogram_Sout_up = neurogram_Sout_up.';
    end
    
    cf_up_mask = CFs_up >= 125 & CFs_up <= 20000;
    CFs_up_plot = CFs_up(cf_up_mask);
    neurogram_Sout_up_plot = neurogram_Sout_up(:,cf_up_mask);
    ...
    neurogram_Sout_down = neurogram_Sout_down .* w_time;
    neurogram_Sout_up   = neurogram_Sout_up   .* w_time;
    ...
    % hard cut frequency band
        ...
        % 
        % R_up   = neurogram_Sout_up(:,idx)';
        % R_down = neurogram_Sout_down(:,idx)';
        % CF_local = CFs(idx);
    ...
    % channel weighting around Fc
        ...
        R_down = neurogram_Sout_down .* w_cf;
        R_up   = neurogram_Sout_up   .* w_cf;
        % R_down = R_down';
        % R_up = R_up';
    ...
    %% Analysis between input signals
        ...
        % %% Test Alignment
        % maxLag_ms = 5;
        % dt = mean(diff(t_Sout_down));
        % fs_out = 1/dt;
        % 
        % maxLag = round(maxLag_ms/1000 * fs_out);
        % 
        % lags = -maxLag:maxLag;
